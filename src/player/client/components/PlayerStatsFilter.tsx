...
export default function PlayerStatsFilter({ onApply }: Props) {
  ...
  const [selectedView, setSelectedView] = useState<string>('AVERAGE');

  const handleClear = useCallback(() => {
    setSelectedSeason(null);
    setSelectedView('AVERAGE');
    onApply?.({ season: null, view: 'AVERAGE' });
  }, [onApply]);

  const handleOnApply = useCallback(
    (event: React.FormEvent<HTMLFormElement>) => {
      event.preventDefault();
      onApply?.({ season: selectedSeason, view: selectedView });
    },
    [onApply, selectedSeason, selectedView],
  );
            ...
            <div className="md:block">
              <DisclosurePanel unmount={false} className="md:!block">
                <div className="p-[20px] pt-0 space-y-5 lg:pt-[20px] lg:px-[30px]">
                  <form onSubmit={handleOnApply} className="space-y-[20px]">
                    ...
                    <div className="space-y-[5px]">
                      ...
                      >
                        <option value="AVERAGE">Promedios por juego</option>
                    ...
                    <div className="flex gap-[10px]">
                      ...
                      >
                        <span className="text-[15px] text-[#0F171F]">
                          Limpiar
                        </span>
                      </button>
                      <button
