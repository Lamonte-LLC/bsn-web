...
/**
 * Initial card of the comparison: title, the search that opens the picker, and the season's scoring leaders as
 * a grid of tiles (six by two on desktop, three by four on phones). Every tile has the same two lines: the
  ...
  return (
    <div className="rounded-[16px] border border-[rgba(15,23,31,0.06)] bg-white px-[16px] pb-[20px] pt-[22px] shadow-[0_12px_32px_rgba(15,23,31,0.08)] lg:px-[44px] lg:pb-[30px] lg:pt-[34px]">
      <div className="text-center">
        <h2 className="text-[20px] leading-[1.1] tracking-[0.3px] text-[#0F171F] lg:text-[24px]">
          Compara de {MIN_COMPARE_PLAYERS} a {MAX_COMPARE_PLAYERS} jugadores del BSN
        </h2>
        <p className="mt-[6px] font-barlow text-[12px] font-medium text-[rgba(15,23,31,0.5)] lg:text-[13px]">Activos o históricos, de cualquier época, lado a lado.</p>
      </div>
      ...
      {suggested.length ? (
        <>
          <div className="mt-[22px] flex flex-wrap items-baseline justify-between gap-x-[12px] gap-y-[4px]">
            <h3 className="text-[17px] tracking-[0.3px] text-[#0F171F]">Líderes de anotación</h3>
            <span className={`${cls.meta} ${cls.tabular}`}>{seasonName ? `${seasonName} · ` : ''}puntos por juego</span>
          </div>
      ...
      <p className="mt-[18px] text-center font-barlow text-[12px] font-medium text-[rgba(15,23,31,0.45)]">Escoge al menos {MIN_COMPARE_PLAYERS} jugadores · la comparación aparece sola</p>
    </div>
  );
}
