...
export const TAB_PILL =
  'inline-flex h-[35px] cursor-pointer items-center justify-center rounded-[100px] border border-[#D5D5D5] bg-white px-[18px] font-special-gothic-condensed-one text-[15px] leading-[1.4] tracking-[0.3px] text-[rgba(0,0,0,0.65)] transition-[background-color,border-color,transform] duration-200 ease-out active:scale-[0.98] motion-reduce:transition-none motion-reduce:active:scale-100 hover:border-[rgba(0,0,0,0.3)] hover:bg-[#FAFAFA] outline-none focus-visible:outline-2 focus-visible:outline-offset-2 focus-visible:outline-[rgba(23,114,217,0.5)] data-selected:border-[#0F171F] data-selected:bg-[#0F171F] data-selected:text-white data-selected:hover:bg-[#0F171F] disabled:cursor-not-allowed disabled:border-[rgba(0,0,0,0.07)] disabled:text-[rgba(0,0,0,0.35)] disabled:hover:bg-white data-disabled:cursor-not-allowed data-disabled:border-[rgba(0,0,0,0.07)] data-disabled:text-[rgba(0,0,0,0.35)] data-disabled:hover:bg-white md:min-w-[150px]';
...
export default function Tabs({ tabs, className = '', right, title, small = false }: { tabs: Array<{ label: string; panel: ReactNode; disabled?: boolean }>; className?: string; right?: ReactNode; title?: ReactNode; small?: boolean }) {
  return (
    <TabGroup className={className}>
      <div className="mb-[20px] flex flex-wrap items-center justify-between gap-[10px]">
        {title}
        <TabList className={`flex flex-wrap gap-[8px] ${small ? '[&>button]:h-[32px] [&>button]:min-w-0 [&>button]:px-[14px] [&>button]:text-[14px]' : ''}`}>
          {tabs.map((t) => (
