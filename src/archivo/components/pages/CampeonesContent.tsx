...
import DynastyTracker from '@/app/archivo/en-numeros/dinastias/DynastyTracker';
...
import { getChampions, getFranchiseMap, getFranchises } from '@/archivo/lib/data';
import { franchiseViewMap } from '@/archivo/lib/franchise-view';
import { hrefs } from '@/archivo/lib/hrefs';
import { cls } from '@/archivo/lib/tokens';
import { dynastyTitles } from './DinastiasContent';

/**
 * Campeones: titles per franchise, then every champion by decade with coach and final series. On the public
 * site the Dynasty Tracker sits between both, so the fan can watch the titles accumulate before the list.
 */
export default function CampeonesContent({ site = false }: { site?: boolean }) {
  const h = hrefs(site);
  ...
  const meta = `${champions.length} títulos desde ${champions[champions.length - 1]?.year}`;

  return (
    <ArchivoShell
      site={site ? { title: 'Campeones', meta, nav: 'campeones' } : undefined}
      hero={
        <div>
    ...
    >
      <section className="mb-[36px] lg:mb-[44px]">
        ...
        <div className="flex flex-wrap gap-[8px]">
          {summary.map(([slug, n]) => {
            ...
            return (
              <Chip key={slug} href={h.franchise(f)}>
      ...
      {site ? (
        <section className="mb-[36px] lg:mb-[44px]">
          <SectionTitle>Dinastías</SectionTitle>
          <DynastyTracker titles={dynastyTitles(champions)} franchises={franchiseViewMap(getFranchises())} site />
        </section>
      ) : null}
                ...
                {list.map((c, i) => {
                  ...
                  return (
                    <li key={c.year} className={`grid min-h-[56px] grid-cols-[56px_1fr] items-center gap-x-[10px] px-[14px] py-[8px] md:grid-cols-[72px_44px_1fr_220px_90px] md:gap-x-[12px] md:px-[20px] ${i ? 'border-t border-[rgba(0,0,0,0.06)]' : ''}`}>
                      <Link href={h.season(c.year)} className={`text-[22px] leading-[1] text-[#0F171F] ${cls.tabular} rounded-[4px] ${cls.focus}`}>
                        {c.year}
                      </Link>
                      ...
                      <span className="min-w-0">
                        <span className="flex items-center gap-[8px]">
                          ...
                          {f ? (
                            <Link href={h.franchise(f)} className={`truncate font-barlow text-[14.5px] font-semibold text-[#0F171F] rounded-[4px] ${cls.focus}`}>
                              {c.fullName}
                            </Link>
                          ) : (
