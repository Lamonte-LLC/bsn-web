...
function YearRow({ y, streak, franchises, h }: { y: MvpChampionYear; streak: 'overlap' | 'none' | null; franchises: Map<string, Franchise>; h: Hrefs }) {
  ...
  return (
    ...
    >
      ...
      <Link href={h.season(y.year)} className={`text-[21px] leading-[1] text-[#0F171F] ${cls.tabular} rounded-[4px] ${cls.focus}`}>
        {y.year}
      </Link>
      <div className="flex min-w-0 flex-col gap-[6px] md:contents">
        <Link href={y.mvp.slug ? h.player(y.mvp.slug) : h.season(y.year)} className={`flex min-w-0 items-center gap-[10px] rounded-[4px] ${cls.focus}`}>
        ...
        <Link href={champF ? h.franchise(champF) : h.season(y.year)} className={`flex min-w-0 items-center gap-[10px] rounded-[4px] ${cls.focus}`}>
...
/** ¿El MVP levanta el trofeo?: year by year, whether the MVP played for the champion. */
export default function MvpYCampeonContent({ site = false }: { site?: boolean }) {
  const h = hrefs(site);
  ...
  return (
    ...
    >
      ...
      <PaperCard className="overflow-hidden">
        ...
        <ol>
          {first.map((y) => (
            <YearRow key={y.year} y={y} streak={streakOf(y.year)} franchises={franchises} h={h} />
          ))}
        </ol>
        {rest.length ? (
