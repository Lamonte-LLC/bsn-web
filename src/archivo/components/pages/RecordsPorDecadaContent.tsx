...
/** Lo mejor de cada década: the best regular-season mark per decade and category. */
export default function RecordsPorDecadaContent({ site = false }: { site?: boolean }) {
  const h = hrefs(site);
                  ...
                  {data.categories.map((c) => {
                    ...
                    return (
                      <td key={c} className="pl-[10px] pt-[16px] pb-[16px]">
                        ...
                        <Link href={h.player(r.slug)} className={`mt-[6px] block max-w-[150px] font-barlow text-[13px] font-semibold leading-[1.25] text-[#0F171F] rounded-[4px] ${cls.focus}`}>
                          {r.name}
                        </Link>
                        <span className={`mt-[5px] flex items-center gap-[5px] font-barlow text-[11.5px] text-[rgba(0,0,0,0.5)] ${cls.tabular}`}>
                          ...
                          <Link href={h.season(r.year ?? '')} className={`hover:text-[#0F171F] rounded-[4px] ${cls.focus}`}>
                            {r.year}
                          </Link>
                        </span>
                      </td>
