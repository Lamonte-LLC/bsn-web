...
/** Récords de temporada y de carrera: a LeaderCard per category with the four runners-up. */
export default function RecordsContent({ site = false }: { site?: boolean }) {
  const h = hrefs(site);
  ...
  const meta = `Serie Regular, mínimo ${records.minGames} juegos`;

  return (
    <ArchivoShell
      site={site ? { title: 'Récords', meta, nav: 'records' } : undefined}
      hero={
        <div>
          ...
          {(Object.keys(SEASON_LABELS) as SeasonRecordKey[]).map((key) => {
            ...
            return (
              <LeaderCard
                ...
                runners={list.slice(1, 5).map((r) => ({ key: `${r.playerId}-${r.year}`, href: h.player(r.slug), label: `${r.name} · ${r.year}`, value: f(r.value) }))}
              />
            );
          })}
        </div>
          ...
          {(Object.keys(CAREER_LABELS) as CareerRecordKey[]).map((key) => {
            ...
            return (
              <LeaderCard
                ...
                runners={list.slice(1, 5).map((r) => ({ key: r.playerId, href: h.player(r.slug), label: r.name, value: fmtInt(r.value) }))}
              />
            );
          })}
        </div>
