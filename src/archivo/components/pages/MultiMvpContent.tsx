...
/** Los que repitieron: the players with more than one MVP on a timeline (desktop) or as year chips (mobile). */
export default function MultiMvpContent({ site = false }: { site?: boolean }) {
  const h = hrefs(site);
          ...
          {players.map((p) => (
            <li key={p.playerId ?? p.name} className="grid h-[47px] grid-cols-[270px_1fr] items-center gap-x-[16px] border-b border-[rgba(0,0,0,0.05)] last:border-b-0">
              <div className="flex min-w-0 items-center gap-[8px]">
                {p.slug ? (
                  <Link href={h.player(p.slug)} className={`truncate font-barlow text-[14px] font-semibold text-[#0F171F] rounded-[4px] ${cls.focus}`}>
                    {p.name}
                  </Link>
                ) : (
          ...
          <PaperCard key={p.playerId ?? p.name} className="px-[16px] py-[14px]">
            <li>
              <div className="flex items-center justify-between gap-[10px]">
                {p.slug ? (
                  <Link href={h.player(p.slug)} className={`min-w-0 truncate font-barlow text-[15px] font-semibold text-[#0F171F] rounded-[4px] ${cls.focus}`}>
                    {p.name}
                  </Link>
                ) : (
              ...
              <div className="mt-[10px] flex flex-wrap items-center gap-[6px]">
                {p.mvps.map((m) => {
                  const f = m.franchiseSlug ? franchises.get(m.franchiseSlug) ?? null : null;
                  return (
                    <Link key={m.year} href={h.season(m.year)} className={`inline-flex h-[30px] items-center gap-[6px] rounded-[7px] border border-[rgba(0,0,0,0.1)] pl-[4px] pr-[9px] font-barlow text-[13px] font-medium text-[rgba(0,0,0,0.7)] ${cls.tabular} ${cls.focus}`}>
