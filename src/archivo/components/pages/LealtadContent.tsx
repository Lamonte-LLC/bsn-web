...
function List({ list, label, total, h }: { list: LoyaltyEntry[]; label: string; total: number; h: Hrefs }) {
      ...
      <ol>
        {list.slice(0, LIMIT).map((e, i) => (
          <li key={e.playerId} className="grid min-h-[56px] grid-cols-[24px_1fr_auto] items-center gap-x-[10px] border-b border-[rgba(0,0,0,0.05)] py-[8px] last:border-b-0">
            ...
            <Link href={h.player(e.slug)} className={`min-w-0 rounded-[4px] ${cls.focus}`}>
...
/** Un solo uniforme: one-club players next to the journeymen. */
export default function LealtadContent({ site = false }: { site?: boolean }) {
  const h = hrefs(site);
      ...
      <PaperCard className="px-[16px] py-[16px] md:px-[26px] md:py-[22px]">
        ...
        <div className="md:hidden">
          <Tabs
            small
            tabs={[
              { label: `Un solo club`, panel: <List list={data.oneClub} label="Un solo club" total={data.oneClub.length} h={h} /> },
              { label: `Trotamundos`, panel: <List list={data.journeymen} label="Trotamundos" total={data.journeymen.length} h={h} /> },
            ]}
          />
        </div>
        <div className="hidden grid-cols-2 gap-[40px] md:grid">
          <List list={data.oneClub} label="Un solo club" total={data.oneClub.length} h={h} />
          <List list={data.journeymen} label="Trotamundos" total={data.journeymen.length} h={h} />
        </div>
      </PaperCard>
    </InsightPage>
  );
